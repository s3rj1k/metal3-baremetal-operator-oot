...
spec:
  ...
  bootMode: UEFI
  online: true
  # BMO reads this Secret for a networkData or value key and claims it with a
  # finalizer. The plugin reads the kickstart from the same value key.
  preprovisioningNetworkDataName: ${KS_SECRET}
  # No boot MAC, no root device hints and no image on purpose. The provisioner
  # refuses to install without the first two, and the image starts the install.
EOF
