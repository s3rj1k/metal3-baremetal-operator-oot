...
if [[ "${cloned}" == true ]]; then
    ...
    # The deployment manifests render from BMO's own config/base, so flux has to
    # read them at the same commit the plugin is compiled against. A branch or tag
    # there would drift from the module pin on its own, so record the exact SHA.
