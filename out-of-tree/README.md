...
The `/serve/` route does not: `/serve/{uid}/{id}` is guarded only by the
unguessable BareMetalHost UID.
...
response that route will hand out.
...
images, the `Makefile` version, and the flux `GitRepository` the manifests under
`deploy/` read BMO's own `config/base` from. That last one is always recorded as
a resolved commit SHA, even when `<ref>` is a branch or a tag, so the manifests
cannot drift from the operator the plugin is compiled against. Undo with
`git checkout go.mod go.sum Dockerfile Makefile deploy`.
