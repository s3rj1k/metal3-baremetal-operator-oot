...
// HostResolver fetches BMH data on demand via the host supplied client.
...
// WWNLink names the by-id link udev creates for a WWN. Ironic reports the 0x
// prefix already and the hint is free text, so both forms are normalized.
...
// specific hint first. Only hints naming a udev link are expressible.
...
func (r *HostResolver) Namespace() string {
	return cmp.Or(r.PodNamespace, core.PodNamespace())
}

// NewHostRef builds a HostRef from a host.
...
// ClearInstallReport drops the recorded verdict, so a reinstall cannot read the
// last run's and finish. Nothing is stamped, BMO records when provisioning began.
...
// InstallStartedAt returns when the host entered provisioning, which BMO stamps
// on the transition and resets for each new run, zero when it has not.
func (r *HostResolver) InstallStartedAt(ctx context.Context, namespace, name string) (time.Time, error) {
	...
	return host.Status.OperationHistory.Provision.Start.Time, nil
}
...
// CallbackReader returns the uncached reader when available, so a Secret written
// moments ago stays visible despite the label filtered informer cache.
...
func (r *HostResolver) HostHardwareData(ctx context.Context, namespace, name string) (*metal3api.HardwareDetails, error) {
	...
	// The CR carries the name and namespace of the host it belongs to.
...
// HostInstallDisk returns the disk spec.rootDeviceHints name, empty when unset.
