...
func TestCallbackHandler(t *testing.T) {
	stub := &stubResolver{uid: testsupport.UID}
	srv := &httpapi.PluginServer{Resolver: stub, Log: logr.Discard()}
	handler := srv.Handler()

	req := httptest.NewRequestWithContext(t.Context(), http.MethodPost, "/callback/bmh-uid/ns/n1", strings.NewReader(`{"done":true}`))
...
func TestParseInstallReportContract(t *testing.T) {
	cases := map[string]struct {
		...
		"no status":         {body: `{}`, wantSucceeded: true},
		...
		"failed with why":   {body: `{"status":"failed","message":"disk sda not found"}`, wantMessage: testsupport.Failure},
...
// A host can post up to CallbackMaxBodyBytes, none of which belongs unbounded in
// a BareMetalHost status field.
