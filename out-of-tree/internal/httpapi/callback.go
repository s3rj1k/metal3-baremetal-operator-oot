...
// ServerResolver is everything the listener needs from the cluster.
...
// PluginServer serves the kickstart and callback routes.
...
// Truncate caps s at n bytes, marking that it was cut.
...
func ParseInstallReport(raw []byte) core.InstallReport {
	...
	if report.Status == "" || slices.ContainsFunc(CallbackSuccessStatuses, func(ok string) bool {
...
// HandleCallback validates the token and persists the body to the per host
// Secret. It makes no provisioning decision, Provision reads the result.
