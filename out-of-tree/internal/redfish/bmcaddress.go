...
// RedfishAddress is a parsed BMC address.
type RedfishAddress struct {
...
// ParseRedfishAddress resolves a BareMetalHost BMC address.
func ParseRedfishAddress(address string) (RedfishAddress, error) {
	if address == "" {
		return RedfishAddress{}, errors.New("BMC address is empty, a redfish-virtualmedia address is required")
	}
	...
	if err != nil {
		return RedfishAddress{}, fmt.Errorf("BMC address %q: %w", address, err)
	}

	if !slices.Contains(SupportedBMCSchemes, access.Type()) {
		return RedfishAddress{}, UnsupportedSchemeError(address, access.Type())
	}
	...
	if endpoint == "" {
		return RedfishAddress{}, fmt.Errorf("BMC address %q resolved to no Redfish endpoint", address)
	}

	systemID, _ := info[RedfishSystemIDKey].(string)

	return RedfishAddress{Endpoint: endpoint, SystemID: systemID}, nil
}
