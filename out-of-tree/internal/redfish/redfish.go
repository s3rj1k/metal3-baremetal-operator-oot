...
// source, virtual media and the system fields inspection reads.
package redfish
...
// MediaStatus is what the virtual media drive currently holds.
...
// MediaIsCD reports whether a media slot accepts CD or DVD images.
...
// InsertableMedia prefers a free CD or DVD slot, then any free slot, then the
// first slot.
...
// WithClient opens a Redfish session and runs fn, logging out afterward.
...
// System resolves the computer system this connection addresses, the one the
// BMC address named or the service's only one when it named none.
...
// SystemInfo is the pair of system fields registration logs. Inspection reads
// the vendor block straight off the system in SystemVendor.
type SystemInfo struct {
	Manufacturer string
	Model        string
}

// WithSystem opens a session, resolves the system, and runs fn.
...
// InsertedMedia returns the slot that currently holds an image, or nil.
...
// Reset applies one reset action to the system.
...
// SecureEraseDrives sanitizes every drive the system reports. BMO calls the
// mode metadata, but Redfish has no metadata only erase and takes the whole disk.
func (c Conn) SecureEraseDrives(ctx context.Context) error {
	return c.WithClient(ctx, func(client *gofish.APIClient) error {
		sys, serr := c.System(client)
		if serr != nil {
			return serr
		...
		for _, d := range Drives(sys) {
			// gofish always sends both parameters, so a BMC that cannot do a
			// cryptographic erase fails here rather than choosing its own way.
			info, eerr := d.SecureErase(0, schemas.CryptographicEraseDataSanitizationType)
			if eerr != nil {
				return fmt.Errorf("drive %s: %w", d.ID, eerr)
			}

			if terr := WaitTask(ctx, client, info); terr != nil {
				return fmt.Errorf("drive %s: %w", d.ID, terr)
			}
		}
...
// InsertMedia attaches an image to the virtual media drive.
...
// MediaStatus reports what the virtual media drive holds.
...
// SystemInfo reads the vendor fields and BIOS version.
...
// Healthy reports whether the system health rolls up to OK. An empty rollup is
// an error, not a failure, because it means the BMC served no data at all.
func (c Conn) Healthy(ctx context.Context) (bool, error) {
	...
	if health == "" {
		return false, errors.New("redfish health: no health data")
	}

	return health == schemas.OKHealth, nil
}
